    ...
    <form action="/mata-kuliah/{{ $mataKuliah->id }}" method="POST">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control @error('kode_mk') is-invalid @enderror" id="kode_mk" name="kode_mk" value="{{ old('kode_mk', $mataKuliah->kode_mk) }}">
                @error('kode_mk')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-6 mb-3">
                <label for="nama_mk" class="form-label">Nama Mata Kuliah</label>
                <input type="text" class="form-control @error('nama_mk') is-invalid @enderror" id="nama_mk" name="nama_mk" value="{{ old('nama_mk', $mataKuliah->nama_mk) }}">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control @error('sks') is-invalid @enderror" id="sks" name="sks" value="{{ old('sks', $mataKuliah->sks) }}">
                @error('sks')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-6 mb-3">
                <label for="semester" class="form-label">Semester</label>
                <input type="number" class="form-control @error('semester') is-invalid @enderror" id="semester" name="semester" value="{{ old('semester', $mataKuliah->semester) }}">
        ...
        <div id="jadwal-container">
            ...
            <div class="row jadwal-item mb-2">
                <div class="col-md-4">
                    @if($index == 0)<label class="form-label">Hari</label>@endif
                ...
                <div class="col-md-3">
                    @if($index == 0)<label class="form-label">Jam Mulai</label>@endif
                    <input type="time" name="jadwals[{{$index}}][jam_mulai]" class="form-control" value="{{$jadwal->jam_mulai}}">
                </div>
                <div class="col-md-3">
                    @if($index == 0)<label class="form-label">Jam Selesai</label>@endif
                    <input type="time" name="jadwals[{{$index}}][jam_selesai]" class="form-control" value="{{$jadwal->jam_selesai}}">
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.remove-jadwal-btn').forEach(addRemoveListener);

        document.getElementById('add-jadwal-btn').addEventListener('click', function () {
            const container = document.getElementById('jadwal-container');
            const newJadwal = container.querySelector('.jadwal-item').cloneNode(true);
            ...
            newJadwal.querySelectorAll('label').forEach(label => label.style.display = 'none');
