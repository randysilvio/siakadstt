...
<div class="container-fluid px-4">
    ...
    <form action="{{ route('administrasi.surat-keputusan.update', $suratKeputusan->id) }}" method="POST">
            ...
            <div class="card-body bg-white p-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="tanggal_terbit" class="form-control rounded-0" value="{{ $suratKeputusan->tanggal_terbit ? $suratKeputusan->tanggal_terbit->format('Y-m-d') : '' }}">
                    </div>
                    <div class="col-12">
        ...
        <div id="segment-konsideran" class="card border-0 shadow-sm rounded-0 mb-4">
            ...
            <div class="card-body bg-white p-4">
                <div class="mb-4">
                    <label class="form-label fw-bold small text-primary">MENIMBANG</label>
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary rounded-0 mt-1 fw-bold" onclick="addRow('container-menimbang', 'menimbang[]')">+ Tambah Baris Menimbang</button>
                </div>

                <div class="mb-4">
                    <label class="form-label fw-bold small text-primary">MENGINGAT</label>
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary rounded-0 mt-1 fw-bold" onclick="addRow('container-mengingat', 'mengingat[]')">+ Tambah Baris Mengingat</button>
                </div>

                 <div class="mb-4">
                    <label class="form-label fw-bold small text-primary">MEMPERHATIKAN</label>
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary rounded-0 mt-1 fw-bold" onclick="addRow('container-memperhatikan', 'memperhatikan[]')">+ Tambah Baris Memperhatikan</button>
                </div>

                <div id="blok-diktum" class="mb-4 mt-5 border-top pt-4">
                    <label class="form-label fw-bold small text-success">MENETAPKAN / MEMUTUSKAN</label>
                    <div id="container-menetapkan">
                         @if(is_array($suratKeputusan->menetapkan) && count($suratKeputusan->menetapkan) > 0)
                            ...
                            <div class="input-group mb-2 row-item">
                                <textarea name="menetapkan[]" class="form-control rounded-0" rows="2">{{ $val }}</textarea>
                                <button type="button" class="btn btn-danger rounded-0 btn-remove"><i class="bi bi-x"></i></button>
                            </div>
                            @endforeach
                        @else
                    ...
                    <button type="button" class="btn btn-sm btn-outline-success rounded-0 mt-1 fw-bold" onclick="addRowTextarea('container-menetapkan', 'menetapkan[]')">+ Tambah Diktum</button>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-0 mb-4 border-start border-primary border-4">
            <div class="card-header bg-white py-3">
                <h6 class="fw-bold mb-0 text-primary uppercase small"><i class="bi bi-people-fill me-2"></i>4. Susunan Panitia & Lampiran Otoritas</h6>
            </div>
            <div class="card-body bg-light p-4">
                
                {{-- TABEL DOSEN (PORTFOLIO LINKED) --}}
                <h6 class="uppercase fw-bold text-primary small mb-3">A. Entitas Dosen (Terhubung Portofolio)</h6>
                <div id="container-dosen" class="mb-4">
                    @if($suratKeputusan->dosens->count() > 0)
                        ...
                        <div class="row g-2 mb-2 row-item">
                            ...
                            <div class="col-md-6">
                                <input type="text" name="jabatan_dalam_surat[]" class="form-control rounded-0 uppercase" value="{{ $taggedDosen->pivot->jabatan_dalam_surat }}" placeholder="Jabatan dlm SK (Misal: Ketua Panitia)">
                            </div>
                            <div class="col-md-1">
                    ...
                    @else
                        <div class="row g-2 mb-2 row-item">
                            ...
                            <div class="col-md-6">
                                <input type="text" name="jabatan_dalam_surat[]" class="form-control rounded-0 uppercase" placeholder="Jabatan dlm SK (Misal: Ketua Panitia)">
                            </div>
                            <div class="col-md-1">
                                <button type="button" class="btn btn-danger w-100 rounded-0 btn-remove"><i class="bi bi-trash"></i></button>
                            </div>
                ...
                <button type="button" class="btn btn-sm btn-outline-primary rounded-0 fw-bold shadow-sm" onclick="addDosenRow()">+ Tambah Dosen</button>


                {{-- [TAMBAHAN BARU] TABEL ENTITAS NON-DOSEN --}}
                <h6 class="uppercase fw-bold text-info small mb-3 mt-5 border-top pt-4">B. Entitas Tendik / Mahasiswa / Eksternal (Teks Bebas)</h6>
                <div id="container-eksternal">
                    @if(!empty($suratKeputusan->panitia_lainnya) && count($suratKeputusan->panitia_lainnya) > 0)
                        @foreach($suratKeputusan->panitia_lainnya as $eksternal)
                        <div class="row g-2 mb-2 row-item">
                            <div class="col-md-5">
                                <input type="text" name="panitia_lainnya_nama[]" class="form-control rounded-0 uppercase" value="{{ $eksternal['nama'] }}" placeholder="Ketik nama lengkap...">
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="panitia_lainnya_jabatan[]" class="form-control rounded-0 uppercase" value="{{ $eksternal['jabatan'] }}" placeholder="Contoh: Anggota (Perwakilan BEM)">
                            </div>
                            <div class="col-md-1">
                                <button type="button" class="btn btn-danger w-100 rounded-0 btn-remove"><i class="bi bi-trash"></i></button>
                            </div>
                ...
                <button type="button" class="btn btn-sm btn-outline-info text-dark rounded-0 mt-2 fw-bold shadow-sm" onclick="addEksternalRow()">+ Tambah Anggota Lainnya</button>
        ...
        <div class="card border-0 shadow-sm rounded-0 mb-5">
            ...
            <div class="card-body bg-white p-4">
                <div class="mb-4">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary rounded-0 mt-1 fw-bold" onclick="addRow('container-tembusan', 'tembusan[]')">+ Tambah Tembusan</button>
                </div>

                <div class="row g-3 border-top pt-3">
                    <div class="col-md-6">
                        <label class="form-label fw-bold small">Jabatan Otoritas *</label>
                        <input type="text" name="penandatangan_jabatan" class="form-control rounded-0 uppercase fw-bold" value="{{ $suratKeputusan->penandatangan_jabatan }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold small">Nama Penandatangan *</label>
                        <input type="text" name="penandatangan_nama" class="form-control rounded-0 fw-bold" value="{{ $suratKeputusan->penandatangan_nama }}" required>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() { toggleSegments(); });
    ...
    function addRowTextarea(containerId, inputName) {
        const container = document.getElementById(containerId);
        const html = `
            <div class="input-group mb-2 row-item">
                <textarea name="${inputName}" class="form-control rounded-0" rows="2"></textarea>
                <button type="button" class="btn btn-danger rounded-0 btn-remove"><i class="bi bi-x"></i></button>
            </div>
        `;
        container.insertAdjacentHTML('beforeend', html);
    ...
    function addEksternalRow() {
        const container = document.getElementById('container-eksternal');
        const html = `
            <div class="row g-2 mb-2 row-item">
                <div class="col-md-5">
                    <input type="text" name="panitia_lainnya_nama[]" class="form-control rounded-0 uppercase" placeholder="Ketik nama lengkap...">
